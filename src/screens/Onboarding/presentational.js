...
class Onboarding extends PureComponent {
  state = {
    ...
    steps: [
      ...
      {
        src: <Illustration
          src={require('./../../../assets/images/emotion.png')} />,
        scaleAnimation: new Animated.Value(0.85),
        opacityAnimation: new Animated.Value(0),
        textParallaxAnimation: new Animated.Value(20),
      },
      {
        src: <Illustration
          src={require('./../../../assets/images/translate.png')}
        />,
        scaleAnimation: new Animated.Value(0.85),
        opacityAnimation: new Animated.Value(0),
        textParallaxAnimation: new Animated.Value(20),
  ...
  _goToNextStep = () => {
    ...
    if(this.state.current === 0) {
      this.setState({
        ...
        Animated.timing(
          this.state.steps[this.state.current].scaleAnimation,
          {
            toValue: 1,
            duration: 450,
        ...
        Animated.timing(
          this.state.steps[this.state.current].opacityAnimation,
          {
            toValue: 1,
            duration: 650,
            useNativeDriver: true
        ...
        Animated.timing(
          this.state.steps[this.state.current].textParallaxAnimation,
          {
            toValue: 0,
            duration: 350,
            useNativeDriver: true
  ...
  render() {
    ...
    <Frame theme="colorful">
      ...
      <View cls="flx-i">
        {current === 0 ? <Fragment>
            <Heading margins="mb3">
              {t('screens.onboarding.welcome', translation)}
            </Heading>
            <Paragraph additionalStyles="f7 mb3">
              {t('screens.onboarding.wantToOnboard', translation)}
            </Paragraph>
            <View cls="flx-i pb2">
              <Illustration
            ...
            </Fragment> : <Animated.View style={{
              transform: [{
                translateX: this.state.moveAnimation
              }]
            }} cls="flx-row flx-i">
                {steps.map((step, index) => index !== 0 && <View
            key={index}
            >
            <Animated.View style={{
              transform: [{
        ...
        <View cls="flx-row jcsb aic mv2 ph2 mta">
          <View cls="flx-i mr2">
            ...
            }} align="center" theme="pill" muted={false}>
                  {(current === 5 || readAll === true) ? t('labels.start', translation) : t('labels.skip', translation) }
            </Button>
          </View>
          <View cls="flx-i ml2">
            {current <= 0 ? <Button handleOnPress={this._goToNextStep} align="center" theme="pill" muted={false}>
              {t('labels.discoverApp', translation)}
            </Button> : <View cls="flx-row aife">
              {steps.map((step, index) => index !== 0 && <View
                 key={index}
                 style={{
