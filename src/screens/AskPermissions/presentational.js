...
class AskPermissions extends PureComponent {
  ...
  _redirectIfPermissionsGranted = async () => {
    if(this.props.permissionsGranted === true) {
      ...
      const { status, expires, permissions } = await Permissions.getAsync(Permissions.AUDIO_RECORDING, Permissions.CAMERA, Permissions.CAMERA_ROLL, Permissions.NOTIFICATIONS)
      if (status === 'granted') {
  ...
  _askPermissions = async () => {
    const { Permissions } = Expo
    const { status, expires, permissions } = await Permissions.askAsync(Permissions.AUDIO_RECORDING, Permissions.CAMERA, Permissions.CAMERA_ROLL, Permissions.NOTIFICATIONS)
    if (status === 'granted') {
      this.props.navigation.navigate('ScreenSuccessPermissions')
    }
  }
  render() {
    ...
    return <Frame theme="clear">
      <Heading margins="mb3">
        {t('screens.askPermissions.title', translation)}
      </Heading>
      <Paragraph additionalStyles="f7 mb3">
        {t('screens.askPermissions.text', translation)}
      </Paragraph>
      <View cls="flx-i">
        <Illustration
          src={require('./../../../assets/images/permission.png')}
        />
      </View>
      <View cls="mta">
        <Button
          handleOnPress={this._askPermissions}
          theme="flat"
          muted={false}
          align="center"
        >
